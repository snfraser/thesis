...
\subsection{Problem complexity metrics}
\label{ss:pcm}
...
\item The \emph{resource usage} metrics $Q_{Ax}$ for various resources $x$ against target values. These would probably be long term metrics, valid over periods of the order of a semester or year. An implicit metric currently in use by the operations team is the TAG assigned Minimum Use Fraction (MUF) \glossary{name={MUF},description={Minimum Use Fraction - a TAG assigned target for program completion},sort={M}}.

\item The \emph{score utility} metric $Q_{SU}$ \notation{name={$Q_{SU}$},description={Score Utility quality metric},sort={Q}}is a metric generated using a weighted sum of the set of metrics used by the scheduler to derive the scores it uses to select groups for scheduling. 
