...
public class QuantumSchedulerTest {
	...
	try {
	    ...
	    // lookahead horizon (sec)
	    long horizon = 1000L*config.getIntValue("horizon", 3600);
	    
	    // time spacing between lookahead candlists (sec).
	    long dtq     = 1000L*config.getIntValue("dtq", 60);

	    // number of sequence tests to perform per lookahead.
	    int nseq     = config.getIntValue("nseq", 1000);

	    QuantumLookaheadScheduler qlas = new QuantumLookaheadScheduler(bcg,
									   history,
									   bam,
									   bxtm,
									   start,
...
// 						       selector);
		...
		while (t < de) {
		    ...
		    // Using QLAS
		    Metric metric = qlas.getScheduleItem(t, env);
