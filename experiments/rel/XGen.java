public class XGen {
    ...
    static double a[] = new double[] {36, 65, 130, 250, 372}; // peak times in minutes
	...
	try {
		...
		for (int it = 0; it < 480; it+=30) {
		    ...
		    double dy = (Math.random()-0.5)*8.0;

		    double erry = 3.0 + (Math.random()-0.5)*4.0;
		...
		for (int iq = 0; iq<q.length; iq++) {
		    ...
		    double fo = f(th/qq, ih);
		    double df = (Math.random()-0.5)*6.0;
		    double errf = 3.0 + (Math.random()-0.5)*4.0;
		    System.err.println("OVER "+ih+" "+qq+" "+((fo+df)/f00)+" "+(errf/f00));
	...
	System.err.println("Calc: f (t="+t+"; h="+hh[ih]+"0 = "+ff);
